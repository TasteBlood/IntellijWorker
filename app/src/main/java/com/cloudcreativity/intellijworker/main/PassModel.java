...
import com.cloudcreativity.intellijworker.utils.PlayAudioDialogUtils;
import com.cloudcreativity.intellijworker.utils.RecordVoiceDialogUtils;
import com.cloudcreativity.intellijworker.utils.SPUtils;
import com.cloudcreativity.intellijworker.utils.ToastUtils;
import com.cloudcreativity.intellijworker.utils.UploadService;
...
public class PassModel {
    ...
    void uploadSuccess(Map<String,Object> data) {
        ...
        HttpUtils.getInstance().addPass(content.get(),voice,images.substring(0,images.length()-1), SPUtils.get().getUid())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new DefaultObserver<String>(baseDialog,true) {
                    @Override
                    public void onSuccess(String t) {
                        ToastUtils.showShortToast(context,"举报成功，我们会尽快处理");
                        context.finish();
                    }
